...
// Environment variables are read with the TOKENSHIM_ prefix;
// dots in keys are replaced with underscores (e.g. scan_paths → TOKENSHIM_SCAN_PATHS).
func NewManager() *Manager {
	v := viper.New()
	v.SetEnvPrefix("TOKENSHIM")
...
// or /etc/tokenshim/, then unmarshals into cfg.
...
func (m *Manager) Load(cfg interface{}) error {
	...
	m.v.AddConfigPath(".")
	m.v.AddConfigPath("/etc/tokenshim/")
...
func (m *Manager) LoadFile(path string, cfg interface{}) error {
	...
	if err := m.v.ReadInConfig(); err != nil {
		if !errors.Is(err, os.ErrNotExist) {
			// viper wraps the underlying error; also check the message for "not found"
			if _, ok := err.(viper.ConfigFileNotFoundError); !ok && !isNotFound(err) {
				return err
			}
		}
		// File absent — proceed with defaults and env vars only.
	}
	return m.v.Unmarshal(cfg)
}
